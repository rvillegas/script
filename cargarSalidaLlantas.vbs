...
'Octubre 26 2015 Cambio pc_vida=0 a pc_vida=1 posiblemente si no puede hacer el calculo porque no tiene informacion lo ponga en 1

sql="update salidas set horometro_final=(select dbo.hor(equipo,fecha)), tipo='SALIDA', pc_vida=1"
cnn.Execute(sql)
...
Else
	...
	uTa="ssadsad"
	Do While Not rs.EOF
		If uEq=rs("equipo") And uTi=rs("tipo_llanta") And ufe=rs("fecha")And uTa=rs("tipo") Then
	...
	Do While Not rs.EOF
		...
		Else 		
			...
			Else
				...
				If rs1.State=1 Then rs1.Close    
				If rs2.State=1 Then rs2.Close   
				'Se copian los horometros anteriores al nuevo registro en todos los casos.
				...
				If rs("tipo")="SALIDA" Then
					'Solo si el registro es una salida cambia los horometros de montaje de las llantas.	
					'Actualiza los horometros de las llantas nuevas por el ultimo hormometro, la cantidad de llantas que se cambiaron/midieron
					rs2.Open "Select * from horometros_llanta where id=" & rs("id") & " order by id, hor",cnn,adOpenDynamic,adLockOptimistic        
					rs2.MoveFirst
					'Si hay algun error y las cantidad es mayor al numero de llantas, lo limita al num total de llantas
					...
					Next
					rs2.Close
					'Calcula la sumatoria de horas de todas las llantas
					...
					If rs("cnt")<>0 Then
						If Not IsNull(rs("prom")) Then
							uprom=rs("prom")
						End If
						'este promedio solo tiene significado si no se cambian todas las llantas
					...
					If sum_ll>=n_ll Then
						...
						If prom_t>2000 Then prom_t=2000
						If prom_t<1000 Then prom_t=1000							
					...
					Else
						...
						End If
					End If
					'Agregado en oct 27: El minimo valor que debe tener pc_vida debe ser num_llantas_cambidas/num_llantas_total, que signifivcaria
					'que solo tienen el 100% de vida las llantas que se pusieron nuevas, las otras estarian en cero.
					min_pv=rs("cnt")/num_llantas(rs("tipo_llanta"))
					If pcv<min_pv Then
						rs("pc_vida")=min_pv
					End If					
				'Fin tipo='SALIDA'

				ElseIf  rs("tipo")="Revisión" Or rs("tipo")="Rotacion" Then
				'En el momento de hacer una revision, no es importante el numero de llantas, se supone que es un dato totalmente cierto
				'para calcular todos los datos se debe hacer asi:
				'sumHor, solo debe cambiar en el caso de cambio de una llanta en este caso se copió pero no cambio nada
				'Prom y promedio tendido en el caso de la medicion seria lo mismo
				'Horas Uso:
					rs2.Open  "SELECT sum(hor)FROM [dbo].[horometros_llanta] where id=" & rs("id"),cnn
					rs2.MoveFirst
					uSumHr=sumHr
					...
					d_pv=uPv-CDbl(rs("pc_vida"))
					u_prom_t=prom_t
					If d_pv>0 Then
						prom_t=(rs("horometro_final")-uHOF)/d_pv
					ElseIf d_pv=0 Then
						prom_t=u_prom_t
					Else
						'la vida aumentó
						...
						prom_t=u_prom_t
					End If
					Debug.WriteLine rs("equipo"),"  ",rs("id"),"   ", d_pv,"  ", uPv,"   ", rs("pc_vida"), " ",prom_t
					rs("prom")= prom_t
					If prom_t>2000 Then prom_t=2000
					If prom_t<1000 Then prom_t=1000					
					rs("prom_tendido")=prom_t
					rs("horas_uso")=n_ll*uHo-sumHr
				End If	'Cierre if de tipo 'SALIDA' o 'Revision'		
			End If 'Cierre if de cambio de equipo, tipo_llanta 

			rs.update
		End If 'Cierre if de tipo='CAMBIO_HOR'
		uPv= CDbl(rs("pc_vida")	)		
		uHof=CDbl(rs("horometro_final"))
		rs.MoveNext
	Loop
End Sub
